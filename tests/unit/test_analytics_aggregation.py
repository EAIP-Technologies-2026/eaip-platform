...
class TestAggregationEngine:
    ...
    class TestAggregate:
        async def test_sum_aggregation(self, seeded_engine: AggregationEngine) -> None:
            ...
            result = await seeded_engine.aggregate("m1", AggregationType.SUM, (start, now + timedelta(hours=1)), interval=86400)
        ...
        async def test_avg_aggregation(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.aggregate("m1", AggregationType.AVG, (start, now + timedelta(hours=1)), interval=86400)
            assert result.points[0].value > 0

        async def test_count_aggregation(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.aggregate("m1", AggregationType.COUNT, (start, now + timedelta(hours=1)), interval=86400)
            assert result.points[0].value == 5.0

        async def test_min_aggregation(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.aggregate("m1", AggregationType.MIN, (start, now + timedelta(hours=1)), interval=86400)
            assert result.points[0].value == 0.0

        async def test_max_aggregation(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.aggregate("m1", AggregationType.MAX, (start, now + timedelta(hours=1)), interval=86400)
            assert result.points[0].value == 40.0

        async def test_latest_aggregation(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.aggregate("m1", AggregationType.LATEST, (start, now + timedelta(hours=1)), interval=86400)
            assert result.points[0].value >= 0
    ...
    class TestComputeDerived:
        async def test_sum_expression(self, seeded_engine: AggregationEngine) -> None:
            ...
            result = await seeded_engine.compute_derived(
                "sum", {"req": "m1", "err": "m2"}, (start, now + timedelta(hours=1)),
            )
            assert len(result) > 0
        ...
        async def test_avg_expression(self, seeded_engine: AggregationEngine) -> None:
            ...
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_derived(
                "avg", {"req": "m1", "err": "m2"}, (start, now + timedelta(hours=1)),
            )
            assert len(result) > 0
        ...
        async def test_diff_expression(self, seeded_engine: AggregationEngine) -> None:
            ...
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_derived(
                "diff", {"req": "m1", "err": "m2"}, (start, now + timedelta(hours=1)),
            )
            assert len(result) > 0
        ...
        async def test_ratio_expression(self, seeded_engine: AggregationEngine) -> None:
            ...
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_derived(
                "ratio", {"req": "m1", "err": "m2"}, (start, now + timedelta(hours=1)),
            )
            assert len(result) > 0

        async def test_raises_on_unsupported_expression(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
    ...
    class TestComputePercentile:
        async def test_p50(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_percentile("m1", 50.0, (start, now + timedelta(hours=1)))
        ...
        async def test_p95(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_percentile("m1", 95.0, (start, now + timedelta(hours=1)))
            assert result["percentile"] == 95.0

        async def test_p99(self, seeded_engine: AggregationEngine) -> None:
            now = datetime.now(timezone.utc)
            start = now - timedelta(hours=1)
            result = await seeded_engine.compute_percentile("m1", 99.0, (start, now + timedelta(hours=1)))
