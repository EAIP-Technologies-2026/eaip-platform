"""Tests for :mod:`eaip.sdk.integration`."""
...
class TestSdkRuntimeModule:
    def test_name(self) -> None:
        module = SdkRuntimeModule()
        assert module.name == "sdk"
    ...
    async def test_start_registers_sdk_health_check(self, kernel: _MockKernel) -> None:
        module = SdkRuntimeModule()
        await module.start(kernel)
        registered = kernel.platform.health.registered[0]
        assert isinstance(registered, SdkHealthCheck)
    ...
    async def test_custom_health_check(self, kernel: _MockKernel) -> None:
        custom = SdkHealthCheck()
        module = SdkRuntimeModule(health_check=custom)
        await module.start(kernel)
        registered = kernel.platform.health.registered[0]
