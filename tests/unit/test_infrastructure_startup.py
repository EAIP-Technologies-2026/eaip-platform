...
class TestPlatformInfrastructureStartup:
    async def test_local_env_initializes_db(self) -> None:
        infra = PlatformInfrastructure(Container(), EventBus(), _settings(Environment.LOCAL))
        with (
            patch(
    ...
    async def test_production_env_initializes_db(self) -> None:
        infra = PlatformInfrastructure(Container(), EventBus(), _settings(Environment.PRODUCTION))
        with (
            patch(
                "eaip.infrastructure.infrastructure.DatabaseConnection.initialize",
                new=AsyncMock(),
