"""Tests for KnowledgeRuntimeModule integration."""
...
class TestKnowledgeRuntimeModule:
    ...
    def test_selector_available_after_start(self) -> None:
        module = KnowledgeRuntimeModule()
        kernel = _MockKernel()
        asyncio.run(module.start(kernel))
    ...
    def test_health_check_registered(self) -> None:
        ...
        kernel = _MockKernel()
        asyncio.run(module.start(kernel))
        assert "knowledge" in kernel.platform.health.registered()
        asyncio.run(module.stop(kernel))
    ...
    def test_capability_registered(self) -> None:
        ...
        assert cap.name == "knowledge:engine"
        asyncio.run(module.stop(kernel))
