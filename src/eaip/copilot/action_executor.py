...
class GovernedActionExecutor:
    ...
    async def plan_action(  # noqa: PLR0917
        self,
        intent: str,
        user: dict[str, Any],
        capability_name: str,
        ...
        resolved_tool = tool_name or "system_health"
        args = arguments or {}
    ...
    async def execute_action(
        ...
        try:
            raw_output: object = await tool.execute(**plan.arguments, user=user)
            parsed_data = {}
            ...
            audit_id = self._log_audit(
                ...
                resource_id=plan.capability_name,
                outcome=AuditOutcome.SUCCESS,
                details={"tool": plan.tool_name, "execution_id": exec_id},
                correlation=correlation,
            )
