...
class TestInitSentry:
    ...
    def test_init_sentry_uses_custom_release(
        self, settings_with_env_override: PlatformSettings
    ) -> None:
        assert settings_with_env_override.sentry.release == "1.2.3"

    def test_init_sentry_uses_custom_environment(
        self, settings_with_env_override: PlatformSettings
    ) -> None:
        assert settings_with_env_override.sentry.environment == "staging"

    def test_init_sentry_defaults_traces_sample_rate_to_zero(
        self, settings_with_dsn: PlatformSettings
    ) -> None:
        assert settings_with_dsn.sentry.traces_sample_rate == 0.0

    def test_init_sentry_defaults_send_default_pii_to_false(
        self, settings_with_dsn: PlatformSettings
    ) -> None:
        assert settings_with_dsn.sentry.send_default_pii is False

    def test_init_sentry_defaults_attach_stacktrace_to_true(
        self, settings_with_dsn: PlatformSettings
    ) -> None:
        assert settings_with_dsn.sentry.attach_stacktrace is True

    @patch("eaip.integrations.sentry._get_sentry_sdk")
    def test_init_sentry_calls_sentry_init_with_config(
        self, mock_get_sdk: MagicMock, settings_with_env_override: PlatformSettings
    ) -> None:
...
class TestAddSentryMiddleware:
    def test_add_sentry_middleware_does_not_raise(
        self, settings_with_dsn: PlatformSettings
    ) -> None:
