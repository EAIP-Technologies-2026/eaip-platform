...
from eaip.workflow.models import WorkflowDefinition, WorkflowStatus, WorkflowStep, WorkflowEdge
...
async def workflow_health(request: Request, _user: dict = Depends(get_current_user)):
    ...
    total = len(wfs)
    active = sum(1 for r in (engine._runs or {}).values() if r.status == WorkflowStatus.RUNNING) if engine else 0
    ...
    failed_today = sum(1 for r in wf_runs if r.status == WorkflowStatus.FAILED and r.created_at.date() == now.date())
    return {
        "status": "healthy",
...
@router.post("")
async def create_workflow(request: Request, body: dict[str, Any], _user: dict = Depends(get_current_user)):
    registry = _get_registry(request)
...
async def update_workflow(request: Request, workflow_id: str, body: dict[str, Any], _user: dict = Depends(get_current_user)):
    registry = _get_registry(request)
    updates = {}
...
async def delete_workflow(request: Request, workflow_id: str, _user: dict = Depends(get_current_user)):
    registry = _get_registry(request)
    try:
...
async def archive_workflow(request: Request, workflow_id: str, _user: dict = Depends(get_current_user)):
    registry = _get_registry(request)
    try:
...
async def duplicate_workflow(request: Request, workflow_id: str, _user: dict = Depends(get_current_user)):
    registry = _get_registry(request)
    try:
...
async def execute_workflow(request: Request, workflow_id: str, _user: dict = Depends(get_current_user)):
    ...
    if engine is None:
        ...
        return {"id": run_id, "workflowId": workflow_id, "status": "completed", "startedAt": datetime.now(timezone.utc).isoformat(), "completedAt": datetime.now(timezone.utc).isoformat(), "duration": 0, "triggeredBy": "manual", "triggeredByType": "manual", "steps": []}
...
async def list_workflow_executions(request: Request, workflow_id: str, _user: dict = Depends(get_current_user)):
    engine = _get_engine(request)
    if engine is None:
        return []
    return [
        {
            ...
            "workflowName": run.definition.name if hasattr(run, "definition") and run.definition else "",
...
async def pause_workflow_execution(request: Request, execution_id: str, _user: dict = Depends(get_current_user)):
    engine = _get_engine(request)
    if engine:
...
async def resume_workflow_execution(request: Request, execution_id: str, _user: dict = Depends(get_current_user)):
    engine = _get_engine(request)
    if engine:
...
async def cancel_workflow_execution(request: Request, execution_id: str, _user: dict = Depends(get_current_user)):
    engine = _get_engine(request)
    if engine:
...
async def retry_workflow_execution(request: Request, execution_id: str, _user: dict = Depends(get_current_user)):
    engine = _get_engine(request)
    if engine is None:
        run_id = f"run-{uuid.uuid4().hex[:8]}"
        return {"id": run_id, "workflowId": "", "status": "completed", "startedAt": datetime.now(timezone.utc).isoformat(), "completedAt": datetime.now(timezone.utc).isoformat(), "duration": 0, "triggeredBy": "manual", "triggeredByType": "manual", "steps": []}
