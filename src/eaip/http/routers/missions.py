...
def _mission_to_dict(m) -> dict[str, Any]:
    return {
        ...
        if m.status == MissionStatus.COMPLETED
            ...
            else (0.1 if m.status == MissionStatus.QUEUED else 0.0)
        ),
        "startedAt": datetime.fromtimestamp(m._started_at, tz=UTC).isoformat()
        if m._started_at
        else None,
        "completedAt": datetime.fromtimestamp(m._completed_at, tz=UTC).isoformat()
        if m._completed_at
        else None,
        "steps": [],
...
async def mission_stats(request: Request, _user: dict = Depends(get_current_user)):
    ...
    if registry:
        ...
        return {
            ...
            "failed": stats["failed"],
            "queued": max(
                0, stats["total"] - stats["running"] - stats["completed"] - stats["failed"]
            ),
        }
...
async def execute_mission(
    request: Request, mission_id: str, _user: dict = Depends(get_current_user)
):
...
async def delete_mission(
    ...
    if registry:
        ...
        if not removed:
            raise HTTPException(
                status_code=HTTP_404_NOT_FOUND, detail=f"Mission {mission_id} not found"
            )
